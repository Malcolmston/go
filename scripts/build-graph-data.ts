...
// Committed cache of the import edges derived from the libraries' Go source, so a
// deploy that excludes those sources (see .vercelignore) keeps its import edges.
const IMPORT_EDGES_CACHE = path.join(REPO_ROOT, 'data', 'import-edges.json');
const OUT_DIRS = [
...
function main(): void {
  ...
  const scannedImportEdges: EdgeOut[] = [];
  let scannedAnySource = false;
  for (const st of libState) {
    if (!fs.existsSync(path.join(st.sourceDir, 'go.mod'))) {
      missingSources.push(st.id);
      continue;
    }
    ...
    for (const p of st.pkgs) {
      ...
      for (const [target, n] of counts) {
        ...
        scannedImportEdges.push({ source: p.importPath, target, kind: 'import', weight: n });
      }
    }
  }

  // Import edges are the one part of the graph derived from the libraries' Go
  // SOURCE rather than from committed data, and the deploy upload deliberately
  // excludes those sources (see .vercelignore) because they run to hundreds of
  // megabytes. Without a fallback the deployed graph silently loses every import
  // edge — which is exactly the set Explore's "dependency edges" hop-distance
  // mode measures, so the feature degrades in production while looking fine
  // locally. So: when the sources ARE present, cache what we scanned into a
  // committed artifact; when they are not, read that artifact back.
  if (scannedAnySource && scannedImportEdges.length) {
    edges.push(...scannedImportEdges);
    importEdgeCount = scannedImportEdges.length;
    try {
      fs.mkdirSync(path.dirname(IMPORT_EDGES_CACHE), { recursive: true });
      fs.writeFileSync(
        IMPORT_EDGES_CACHE,
        JSON.stringify({ note: 'Generated by scripts/build-graph-data.ts from Go source. Committed so a deploy without the submodule sources keeps its import edges. Regenerate by running the script with the submodules checked out.', edges: scannedImportEdges }, null, 0) + '\n',
      );
    } catch (err) {
      console.warn(`build-graph-data: could not write ${IMPORT_EDGES_CACHE}: ${(err as Error).message}`);
    }
  } else {
    let cached: EdgeOut[] = [];
    try {
      const parsed = JSON.parse(fs.readFileSync(IMPORT_EDGES_CACHE, 'utf8')) as { edges?: unknown };
      if (Array.isArray(parsed.edges)) {
        cached = parsed.edges.filter(
          (e): e is EdgeOut =>
            !!e && typeof e === 'object' &&
            typeof (e as EdgeOut).source === 'string' &&
            typeof (e as EdgeOut).target === 'string',
        );
      }
    } catch {
      cached = [];
    }
    // Only keep edges whose endpoints are still real nodes, so a stale cache
    // cannot invent packages that no longer exist.
    const usable = cached.filter((e) => packageIds.has(e.source) && packageIds.has(e.target));
    edges.push(...usable);
    importEdgeCount = usable.length;
    console.warn(
      `build-graph-data: Go sources absent (${missingSources.length} libraries); ` +
        `reused ${usable.length} import edges from ${path.relative(REPO_ROOT, IMPORT_EDGES_CACHE)}` +
        (cached.length && usable.length !== cached.length ? ` (${cached.length - usable.length} stale, dropped)` : ''),
    );
  }
