// graphstore.js — pure query functions over the loaded package-connection graph.
//
...
export function packages({ library, search, first = 50 } = {}) {
  const graph = getGraph();
...
export function subgraph(library) {
  const graph = getGraph();
