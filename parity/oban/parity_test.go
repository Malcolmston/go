// Package parity drives the upstream (Elixir Oban) and Go (github.com/malcolmston/oban)
...
func TestParity(t *testing.T) {
	...
	if mixPath == "" {
		t.Skip("elixir/mix not found on PATH: cannot run the upstream oban runner")
	}
	if lookPath("elixir") == "" {
		t.Skip("elixir not found on PATH: cannot run the upstream oban runner")
	}
...
func buildUpstream(t *testing.T, root, mixPath string) string {
	...
	bin := filepath.Join(dir, "oban_parity")
...
func buildGo(t *testing.T, root string) string {
	t.Helper()
	bin := filepath.Join(t.TempDir(), "go-oban-runner")
...
func writeParityJSON(t *testing.T, root string, cases []testCase, total, matched, mismatched, deviations int) {
	...
	report := map[string]any{
		...
		"byGroup":                byGroup,
		"notComparableWithoutDB": "queues, plugins, telemetry and the whole execution engine require PostgreSQL; see COVERAGE.md",
	}
...
func goModuleVersion(t *testing.T, root string) string {
	...
	for _, line := range strings.Split(string(raw), "\n") {
		...
		if strings.HasPrefix(line, "require github.com/malcolmston/oban ") ||
			strings.HasPrefix(line, "github.com/malcolmston/oban ") {
			fields := strings.Fields(line)
			for i, f := range fields {
				if f == "github.com/malcolmston/oban" && i+1 < len(fields) {
					return "github.com/malcolmston/oban@" + fields[i+1]
				}
			}
		}
	}
