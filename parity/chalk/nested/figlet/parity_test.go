// Package parity drives the upstream (Node/figlet@1.11.4) and Go
// (github.com/malcolmston/chalk/figlet) runners over the same case files and
// compares their answers. Upstream is the oracle: nothing here hand-writes an
// expected value.
//
// figlet lives inside the chalk sub-repo but ports a different npm package, so
// it is scored by this nested harness rather than by parity/chalk/ (which scores
// chalk itself). Both runners are fed the FIGfonts in this directory's fonts/,
// so the oracle and the port read byte-identical font files.
//
// Run with:
//
//	cd parity/chalk/nested/figlet && GOWORK=off go test .
//
...
// show renders a value for test output. ASCII art is multi-line, so a string is
// printed as its own indented block rather than a single quoted line.
...
func goModuleVersion() string {
	...
	if m := modLine.FindSubmatch(raw); m != nil {
		return "github.com/malcolmston/chalk/figlet " + string(m[1])
	}
	return "unknown"
}
...
func TestParity(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join("node", "node_modules", "figlet", "package.json")); err != nil {
	...
	fonts, err := filepath.Abs("fonts")
	if err != nil {
		t.Fatalf("resolve fonts dir: %v", err)
	}
	if _, err := os.Stat(filepath.Join(fonts, "Standard.flf")); err != nil {
		t.Fatalf("fonts/ is missing its FIGfonts: %v", err)
	}
	...
	nodeCmd.Dir = "node"
	nodeCmd.Env = append(os.Environ(), "FIGLET_FONTS="+fonts)
	up := startRunner(t, "node", nodeCmd)

	goCmd := exec.Command(bin)
	goCmd.Env = append(os.Environ(), "GOWORK=off", "FIGLET_FONTS="+fonts)
...
func assertUpstreamVersion(t *testing.T, node string) {
	...
	raw, err := os.ReadFile(filepath.Join("node", "node_modules", "figlet", "package.json"))
	if err != nil {
		t.Skipf("cannot read installed figlet: %v", err)
	}
	var pkg struct{ Version string }
	if err := json.Unmarshal(raw, &pkg); err != nil {
		t.Fatalf("parse installed figlet package.json: %v", err)
	}
	if pkg.Version != want {
		t.Fatalf("installed figlet is %s, cases pin %s", pkg.Version, want)
	}
}
