// Package parity drives the upstream (Node/chalk@5.3.0) and Go
// (github.com/malcolmston/chalk) runners over the same case files and compares
...
func goModuleVersion() string {
	...
	if m := modLine.FindSubmatch(raw); m != nil {
		return "github.com/malcolmston/chalk " + string(m[1])
	}
	return "unknown"
}
...
func TestParity(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join("node", "node_modules", "chalk", "package.json")); err != nil {
...
func assertUpstreamVersion(t *testing.T, node string) {
	...
	raw, err := os.ReadFile(filepath.Join("node", "node_modules", "chalk", "package.json"))
	if err != nil {
		t.Skipf("cannot read installed chalk: %v", err)
	}
	var pkg struct{ Version string }
	if err := json.Unmarshal(raw, &pkg); err != nil {
		t.Fatalf("parse installed chalk package.json: %v", err)
	}
	if pkg.Version != want {
		t.Fatalf("installed chalk is %s, cases pin %s", pkg.Version, want)
	}
}
