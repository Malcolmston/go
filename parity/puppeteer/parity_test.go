// Package puppeteerparity drives the Node oracle runner and the
// github.com/malcolmston/puppeteer runner over identical cases and compares
// their answers. No expectation is hand-written.
//
// The oracle is deliberately not npm's puppeteer: this port contains no CDP, no
// websocket, no Evaluate and no Screenshot, so puppeteer's browser-automation
// surface has nothing on the Go side to compare against and is scored as
// missing in COVERAGE.md. What the port actually is — net/http plus an original
// HTML tokenizer/parser and a CSS selector engine — is compared against
// cheerio@1.0.0, tough-cookie@5.1.2 and node's built-in fetch/URL.
// No browser is launched or downloaded; every HTTP case runs on loopback.
package puppeteerparity
...
func goModuleVersion() string {
	out, err := exec.Command("go", "list", "-m", "-f", "{{.Path}} {{.Version}}", "github.com/malcolmston/puppeteer").Output()
	if err != nil {
		return "github.com/malcolmston/puppeteer (version unknown)"
	}
...
func TestParity(t *testing.T) {
	...
	for _, dep := range []string{"cheerio", "tough-cookie"} {
		if _, err := os.Stat(filepath.Join("node", "node_modules", dep)); err != nil {
			t.Skipf("node/node_modules/%s is missing; run `npm install` in parity/puppeteer/node", dep)
		}
	}
	...
	sc := score{
		...
		Generated: "go test ./parity/puppeteer/",
