...
| cases | 151 (`cases/*.json`) — 149 match, 0 differ, 2 declared deviations = **100.0 %** |
...
**Case parity = 149 match + 2 declared deviations / 151 = 100.0 %** (see
`parity.json`, regenerated by `go test`). The two deviations are
`params-multi-key-order` (Go maps carry no insertion order) and
`headers-default-user-agent` (the port advertises `axios/<port-version>`, not the
pinned upstream `axios/1.7.9`); both are documented in the port's
`API-DEVIATIONS.md`.

> **Status note (parity pass).** The behavioural divergences catalogued in the
> sections below were the state at 66.9 %. All except the two declared deviations
> have since been fixed in the port: bracketed/nested/array-of-object param
> encoding and array default format, `baseURL` path preservation, params appended
> after an existing query, request `Content-Type` selection (method-default for
> raw bodies, `;charset=utf-8` for `url.Values`, no HTML escaping in JSON, the
> multipart trailing CRLF), auto JSON parsing via `Response.Data`, `statusText` as
> the reason phrase, the full `Accept`/`Accept-Encoding` defaults, null-header
> removal, verbatim propagation of interceptor/transform errors, and the axios
> error-code mapping (`ERR_BAD_REQUEST` for 4xx, `ECONNABORTED`/`ECONNREFUSED`/
> `ERR_FR_TOO_MANY_REDIRECTS`/`ERR_INVALID_URL`, `maxRedirects` 0/negative
> semantics via a `*int`).
