// Package parity scores github.com/malcolmston/socketio/engineio against the
// npm package it ports, engine.io-parser (plus engine.io for the protocol
// revision), by running both over an identical set of cases and comparing their
// answers. See ../../../HARNESS.md.
//
// This is a NESTED harness: parity/socket.io/ scores socket.io itself and only
// incidentally touches the Engine.IO framing layer. Everything here is pure and
// socket-free — packet framing, the polling payload, and the errors both
// implementations must produce for malformed input.
package parity
...
func TestParity(t *testing.T) {
	...
	if err != nil {
		t.Skip("node not found in PATH; skipping engineio parity (Go-only checkout)")
	}
	for _, dep := range []string{"engine.io-parser", "engine.io"} {
		if _, err := os.Stat(filepath.Join("node", "node_modules", dep, "package.json")); err != nil {
			t.Skipf("upstream %s not installed; run `npm install` in parity/socket.io/nested/engineio/node", dep)
		}
	}
	...
	sc := &score{
		...
		Generated: "GOWORK=off go test ./parity/socket.io/nested/engineio/",
	...
	t.Logf("socket.io/engineio parity: %d/%d compared cases match (%.1f%%), %d mismatches, %d declared deviations, %d cases total",
