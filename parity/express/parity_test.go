// Package parity drives the upstream (Node/express) runner and the Go
// (github.com/malcolmston/express) runner over an identical set of cases and
...
func TestParity(t *testing.T) {
	...
	if err != nil {
		t.Skip("node not found in PATH; skipping express parity (Go-only checkout)")
	}
	if _, err := os.Stat(filepath.Join("node", "node_modules", "express", "package.json")); err != nil {
		t.Skip("upstream express not installed; run `npm install` in parity/express/node")
	}
	...
	env := append(os.Environ(), "PARITY_FIXTURES="+fixtures)
	...
	sc := &score{
		...
		Generated: "go test ./parity/express/",
	...
	t.Logf("express parity: %d/%d cases match (%.1f%%), %d mismatches, %d declared deviations",
