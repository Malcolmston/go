// Package parity drives the upstream (Node/slugify) runner and the Go
// (github.com/malcolmston/express/slugify) runner over an identical set of
// cases and compares their answers. See ../../../HARNESS.md.
//
// This is a NESTED harness: the oracle is slugify, the package the Go
...
func TestParity(t *testing.T) {
	...
	if err != nil {
		t.Skip("node not found in PATH; skipping express/slugify parity (Go-only checkout)")
	}
	if _, err := os.Stat(filepath.Join("node", "node_modules", "slugify", "package.json")); err != nil {
		t.Skip("upstream slugify not installed; run `npm install` in parity/express/nested/slugify/node")
	}
	...
	sc := &score{
		Library:   "express/slugify",
		Upstream:  upstream,
		GoModule:  goModuleVersion(),
		GoPackage: "github.com/malcolmston/express/slugify",
		GoVersion: strings.TrimSpace(runOut("go", "version")),
		Generated: "GOWORK=off go test ./parity/express/nested/slugify/",
	...
	t.Logf("express/slugify parity: %d/%d compared cases match (%.1f%%), %d undeclared mismatches, %d observed deviations of %d declared (%d cases total)",
