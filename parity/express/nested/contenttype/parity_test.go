// Package parity drives the upstream Node (content-type) runner and the Go
// (github.com/malcolmston/express/contenttype) runner over an identical set of
// cases and compares their answers. See ../../../HARNESS.md.
//
// This is a *nested* harness: the oracle is the nested package's own upstream
// (content-type), not express, and it is pinned separately in node/package.json.
package parity
...
const (
	library     = "express/contenttype"
	npmPackage  = "content-type"
...
func TestParity(t *testing.T) {
	...
	sc := &score{
		...
		GoPackage: "github.com/malcolmston/express/contenttype",
		GoVersion: strings.TrimSpace(runOut("go", "version")),
		Generated: "GOWORK=off go test ./parity/express/nested/contenttype/",
