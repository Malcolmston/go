// Package parity drives the upstream (Node/html-entities) runner and the Go
// (github.com/malcolmston/express/htmlentities) runner over an identical set of cases
...
const (
	library     = "express/htmlentities"
	upstreamPkg = "html-entities"
	generatedBy = "go test ./parity/express/nested/htmlentities/"
...
func TestParity(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join("node", "node_modules", upstreamPkg, "package.json")); err != nil {
		t.Skipf("upstream %s not installed; run `npm install` in parity/express/nested/htmlentities/node", upstreamPkg)
	}
	...
	sc := &score{
		Library:   library,
		Upstream:  upstream,
		GoPackage: "github.com/malcolmston/express/htmlentities",
