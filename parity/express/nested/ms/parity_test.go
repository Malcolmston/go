// Package parity drives the upstream (npm ms) and Go
// (github.com/malcolmston/express/ms) runners over an identical set of cases and
// compares their answers. Upstream is the oracle: nothing here hand-writes an
// expectation.
//
// Run with:
//
//	cd parity/express/nested/ms && GOWORK=off go test .
//
// GOWORK=off is required: this is a separate module, outside the repo workspace.
package parity
...
const (
	// libName is what parity.json reports and what the log line says.
	libName = "express/ms"
	// goPkgPath is the Go module path whose version parity.json records.
	goPkgPath = "github.com/malcolmston/express"
	// npmPkg is the node_modules directory the upstream runner needs.
	npmPkg = "ms"
...
func TestParity(t *testing.T) {
	...
	if err != nil {
		t.Skipf("node not found in PATH; skipping cross-language parity for %s", libName)
	}
	if _, err := os.Stat(filepath.Join("node", "node_modules", npmPkg)); err != nil {
		t.Skipf("node/node_modules/%s missing; run `npm install` in this harness's node/ directory", npmPkg)
	}
	...
	t.Logf("%s parity: %d/%d cases match (%.2f%%), %d deviations, upstream %s, port %s",
		libName, s.Match, compared, s.ParityPct, s.Deviations, s.Upstream, s.GoModule)
}
