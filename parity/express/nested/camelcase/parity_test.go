// Package parity drives the upstream (Node/camelcase) runner and the Go
// (github.com/malcolmston/express/camelcase) runner over an identical set of
// cases and compares their answers. See ../../../HARNESS.md.
//
// This is a NESTED harness: the oracle is camelcase, the package the Go
...
func TestParity(t *testing.T) {
	...
	if err != nil {
		t.Skip("node not found in PATH; skipping express/camelcase parity (Go-only checkout)")
	}
	if _, err := os.Stat(filepath.Join("node", "node_modules", "camelcase", "package.json")); err != nil {
		t.Skip("upstream camelcase not installed; run `npm install` in parity/express/nested/camelcase/node")
	}
	...
	sc := &score{
		Library:   "express/camelcase",
		Upstream:  upstream,
		GoModule:  goModuleVersion(),
		GoPackage: "github.com/malcolmston/express/camelcase",
		GoVersion: strings.TrimSpace(runOut("go", "version")),
		Generated: "GOWORK=off go test ./parity/express/nested/camelcase/",
	...
	t.Logf("express/camelcase parity: %d/%d compared cases match (%.1f%%), %d undeclared mismatches, %d observed deviations of %d declared (%d cases total)",
