// Package parity drives the upstream (Node/filesize) runner and the Go
// (github.com/malcolmston/express/filesize) runner over an identical set of
// cases and compares their answers. See ../../../HARNESS.md.
//
// This is a NESTED harness: the oracle is filesize, the package the Go
...
func TestParity(t *testing.T) {
	...
	if err != nil {
		t.Skip("node not found in PATH; skipping express/filesize parity (Go-only checkout)")
	}
	if _, err := os.Stat(filepath.Join("node", "node_modules", "filesize", "package.json")); err != nil {
		t.Skip("upstream filesize not installed; run `npm install` in parity/express/nested/filesize/node")
	}
	...
	sc := &score{
		Library:   "express/filesize",
		Upstream:  upstream,
		GoModule:  goModuleVersion(),
		GoPackage: "github.com/malcolmston/express/filesize",
		GoVersion: strings.TrimSpace(runOut("go", "version")),
		Generated: "GOWORK=off go test ./parity/express/nested/filesize/",
	...
	t.Logf("express/filesize parity: %d/%d compared cases match (%.1f%%), %d undeclared mismatches, %d observed deviations of %d declared (%d cases total)",
