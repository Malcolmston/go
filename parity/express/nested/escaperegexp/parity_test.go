// Package parity drives the upstream (Node/escape-string-regexp) runner and the Go
// (github.com/malcolmston/express/escaperegexp) runner over an identical set of cases
// and compares their answers. See ../../../HARNESS.md.
//
// Both runners are long-lived subprocesses speaking JSON Lines on stdio; each is
// started once and every case is streamed through it in order.
package parity
...
const (
	library     = "express/escaperegexp"
	upstreamPkg = "escape-string-regexp"
	generatedBy = "go test ./parity/express/nested/escaperegexp/"
...
type score struct {
	...
	DeviationC []string       `json:"deviationCases"`
}
...
func TestParity(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join("node", "node_modules", upstreamPkg, "package.json")); err != nil {
		t.Skipf("upstream %s not installed; run `npm install` in parity/express/nested/escaperegexp/node", upstreamPkg)
	}
	...
	sc := &score{
		Library:   library,
		Upstream:  upstream,
		GoPackage: "github.com/malcolmston/express/escaperegexp",
	...
	for _, c := range cases {
		...
		// Outcome is three-way: a case marked "deviation" that really does
		// differ is counted apart from both a match and a bug, and is excluded
		// from the parity denominator (see HARNESS.md).
		ok, deviated := true, false
		...
		case deviated:
			sc.Deviations++
			g.Deviations++
			sc.DeviationC = append(sc.DeviationC, c.ID)
		case ok:
			sc.Match++
			g.Match++
		default:
	...
	sc.Compared = sc.Match + sc.Mismatch
	if sc.Compared > 0 {
	...
	t.Logf("%s parity: %d/%d compared cases match (%.1f%%), %d mismatches, %d declared deviations, %d total",
		library, sc.Match, sc.Compared, sc.Percent, sc.Mismatch, sc.Deviations, sc.Total)
}
