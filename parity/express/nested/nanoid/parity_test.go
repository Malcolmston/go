// Package parity drives the upstream (Node/nanoid) runner and the Go
// (github.com/malcolmston/express/nanoid) runner over an identical set of
// cases and compares their answers. See ../../../HARNESS.md.
//
// This is a NESTED harness: the oracle is nanoid, the package the Go
...
func TestParity(t *testing.T) {
	...
	if err != nil {
		t.Skip("node not found in PATH; skipping express/nanoid parity (Go-only checkout)")
	}
	if _, err := os.Stat(filepath.Join("node", "node_modules", "nanoid", "package.json")); err != nil {
		t.Skip("upstream nanoid not installed; run `npm install` in parity/express/nested/nanoid/node")
	}
	...
	sc := &score{
		Library:   "express/nanoid",
		Upstream:  upstream,
		GoModule:  goModuleVersion(),
		GoPackage: "github.com/malcolmston/express/nanoid",
		GoVersion: strings.TrimSpace(runOut("go", "version")),
		Generated: "GOWORK=off go test ./parity/express/nested/nanoid/",
	...
	t.Logf("express/nanoid parity: %d/%d compared cases match (%.1f%%), %d undeclared mismatches, %d observed deviations of %d declared (%d cases total)",
