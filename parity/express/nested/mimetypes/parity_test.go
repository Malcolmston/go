// Package parity drives the upstream Node (mime-types) runner and the Go
// (github.com/malcolmston/express/mimetypes) runner over an identical set of
// cases and compares their answers. See ../../../HARNESS.md.
//
// This is a *nested* harness: the oracle is the nested package's own upstream
// (mime-types), not express, and it is pinned separately in node/package.json.
package parity
...
const (
	library     = "express/mimetypes"
	npmPackage  = "mime-types"
...
func TestParity(t *testing.T) {
	...
	sc := &score{
		...
		GoPackage: "github.com/malcolmston/express/mimetypes",
		GoVersion: strings.TrimSpace(runOut("go", "version")),
		Generated: "GOWORK=off go test ./parity/express/nested/mimetypes/",
