// Package parity drives the upstream (Node/striptags) runner and the Go
// (github.com/malcolmston/express/striptags) runner over an identical set of cases
...
const (
	library     = "express/striptags"
	upstreamPkg = "striptags"
	generatedBy = "go test ./parity/express/nested/striptags/"
...
func TestParity(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join("node", "node_modules", upstreamPkg, "package.json")); err != nil {
		t.Skipf("upstream %s not installed; run `npm install` in parity/express/nested/striptags/node", upstreamPkg)
	}
	...
	sc := &score{
		Library:   library,
		Upstream:  upstream,
		GoPackage: "github.com/malcolmston/express/striptags",
