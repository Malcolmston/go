// Package parity drives the upstream (Node/lodash (deburr)) runner and the Go
// (github.com/malcolmston/express/deburr) runner over an identical set of
// cases and compares their answers. See ../../../HARNESS.md.
//
// This is a NESTED harness: the oracle is lodash@4.17.21 (its `deburr` method), the utility the Go
...
func TestParity(t *testing.T) {
	...
	if err != nil {
		t.Skip("node not found in PATH; skipping express/deburr parity (Go-only checkout)")
	}
	if _, err := os.Stat(filepath.Join("node", "node_modules", "lodash", "package.json")); err != nil {
		t.Skip("upstream lodash not installed; run `npm install` in parity/express/nested/deburr/node")
	}
	...
	sc := &score{
		Library:   "express/deburr",
		Upstream:  upstream,
		GoModule:  goModuleVersion(),
		GoPackage: "github.com/malcolmston/express/deburr",
		GoVersion: strings.TrimSpace(runOut("go", "version")),
		Generated: "GOWORK=off go test ./parity/express/nested/deburr/",
	...
	t.Logf("express/deburr parity: %d/%d compared cases match (%.1f%%), %d undeclared mismatches, %d observed deviations of %d declared (%d cases total)",
