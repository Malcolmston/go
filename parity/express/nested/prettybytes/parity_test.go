// Package parity drives the upstream (Node/pretty-bytes) runner and the Go
// (github.com/malcolmston/express/prettybytes) runner over an identical set of
// cases and compares their answers. See ../../../HARNESS.md.
//
// This is a NESTED harness: the oracle is pretty-bytes, the package the Go
// sub-package ports, not express. It is a peer of ../../parity.json, never a
// replacement.
package parity
...
// score is the shape of parity.json.
//
// A case marked "deviation" whose behaviour really does differ is counted in
// Deviations and excluded from Compared: HARNESS.md requires it to be counted
// separately from a bug, and a deliberate difference must not leave the suite
// permanently red. Mismatch therefore holds only UNDECLARED differences, and
// the test fails if and only if it is non-zero. Percent is Match/Compared.
//
// Declared counts the deviation tags in cases/; when it exceeds Deviations, a
// tag describes a difference that no longer happens and should be deleted.
type score struct {
...
func TestParity(t *testing.T) {
	...
	if err != nil {
		t.Skip("node not found in PATH; skipping express/prettybytes parity (Go-only checkout)")
	}
	if _, err := os.Stat(filepath.Join("node", "node_modules", "pretty-bytes", "package.json")); err != nil {
		t.Skip("upstream pretty-bytes not installed; run `npm install` in parity/express/nested/prettybytes/node")
	}
	...
	// A fixed locale keeps Intl-dependent upstream formatting deterministic.
	env := append(os.Environ(), "LC_ALL=en_US.UTF-8", "LANG=en_US.UTF-8", "TZ=UTC")
	...
	goCmd.Env = append(env, "GOWORK=off")
	gorun := startRunner(t, "go", goCmd)

	sc := &score{
		Library:   "express/prettybytes",
		Upstream:  upstream,
		GoModule:  goModuleVersion(),
		GoPackage: "github.com/malcolmston/express/prettybytes",
		GoVersion: strings.TrimSpace(runOut("go", "version")),
		Generated: "GOWORK=off go test ./parity/express/nested/prettybytes/",
	...
	for _, c := range cases {
		...
		t.Run(c.ID, func(t *testing.T) {
			// report is t.Errorf for an undeclared difference and t.Logf for a
			// declared one, so a deliberate deviation is recorded without
			// turning the suite red.
			report := t.Errorf
			prefix := ""
			if c.Deviation != "" {
				report = t.Logf
				prefix = "declared deviation (" + c.Deviation + "): "
			}
			switch {
			case upErr != nil:
				agree = false
			...
			case upRep.OK != goRep.OK:
				agree = false
				report("%sok mismatch: upstream ok=%v (%s), go ok=%v (%s)",
					prefix, upRep.OK, upRep.Error, goRep.OK, goRep.Error)
				return
			case !upRep.OK:
			...
			if !reflect.DeepEqual(uv, gv) {
				agree = false
				report("%svalue mismatch\n  upstream: %s\n        go: %s", prefix, pretty(uv), pretty(gv))
			}
		})
		...
		case agree:
			...
			g.Match++
			if c.Deviation != "" {
				// The declared difference did not happen: the tag is stale.
				sc.Stale = append(sc.Stale, c.ID)
			}
		case c.Deviation != "":
			sc.Deviations++
			g.Deviation++
	...
	sc.Compared = sc.Total - sc.Deviations
	if sc.Compared > 0 {
	...
	t.Logf("express/prettybytes parity: %d/%d compared cases match (%.1f%%), %d undeclared mismatches, %d observed deviations of %d declared (%d cases total)",
		sc.Match, sc.Compared, sc.Percent, sc.Mismatch, sc.Deviations, sc.Declared, sc.Total)
	if len(sc.Stale) > 0 {
		t.Logf("stale deviation tags (the declared difference no longer happens): %s",
			strings.Join(sc.Stale, ", "))
	}
}
