// Package parity drives the upstream Node (accepts) runner and the Go
// (github.com/malcolmston/express/accepts) runner over an identical set of
// cases and compares their answers. See ../../../HARNESS.md.
//
// This is a *nested* harness: the oracle is the nested package's own upstream
// (accepts), not express, and it is pinned separately in node/package.json.
package parity
...
func TestParity(t *testing.T) {
	...
	sc := &score{
		...
		GoPackage: "github.com/malcolmston/express/accepts",
		GoVersion: strings.TrimSpace(runOut("go", "version")),
		Generated: "GOWORK=off go test ./parity/express/nested/accepts/",
