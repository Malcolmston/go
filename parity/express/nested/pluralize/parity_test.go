// Package parity drives the upstream (Node/pluralize) runner and the Go
// (github.com/malcolmston/express/pluralize) runner over an identical set of
// cases and compares their answers. See ../../../HARNESS.md.
//
// This is a NESTED harness: the oracle is pluralize, the package the Go
...
func TestParity(t *testing.T) {
	...
	if err != nil {
		t.Skip("node not found in PATH; skipping express/pluralize parity (Go-only checkout)")
	}
	if _, err := os.Stat(filepath.Join("node", "node_modules", "pluralize", "package.json")); err != nil {
		t.Skip("upstream pluralize not installed; run `npm install` in parity/express/nested/pluralize/node")
	}
	...
	sc := &score{
		Library:   "express/pluralize",
		Upstream:  upstream,
		GoModule:  goModuleVersion(),
		GoPackage: "github.com/malcolmston/express/pluralize",
		GoVersion: strings.TrimSpace(runOut("go", "version")),
		Generated: "GOWORK=off go test ./parity/express/nested/pluralize/",
	...
	t.Logf("express/pluralize parity: %d/%d compared cases match (%.1f%%), %d undeclared mismatches, %d observed deviations of %d declared (%d cases total)",
