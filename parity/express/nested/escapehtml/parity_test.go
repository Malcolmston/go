// Package parity drives the upstream (Node/escape-html) runner and the Go
// (github.com/malcolmston/express/escapehtml) runner over an identical set of cases
...
const (
	library     = "express/escapehtml"
	upstreamPkg = "escape-html"
	generatedBy = "go test ./parity/express/nested/escapehtml/"
...
func TestParity(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join("node", "node_modules", upstreamPkg, "package.json")); err != nil {
		t.Skipf("upstream %s not installed; run `npm install` in parity/express/nested/escapehtml/node", upstreamPkg)
	}
	...
	sc := &score{
		Library:   library,
		Upstream:  upstream,
		GoPackage: "github.com/malcolmston/express/escapehtml",
