// Package parity drives the upstream (Node/sanitize-html) runner and the Go
// (github.com/malcolmston/express/sanitizehtml) runner over an identical set of cases
...
const (
	library     = "express/sanitizehtml"
	upstreamPkg = "sanitize-html"
	generatedBy = "go test ./parity/express/nested/sanitizehtml/"
...
func TestParity(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join("node", "node_modules", upstreamPkg, "package.json")); err != nil {
		t.Skipf("upstream %s not installed; run `npm install` in parity/express/nested/sanitizehtml/node", upstreamPkg)
	}
	...
	sc := &score{
		Library:   library,
		Upstream:  upstream,
		GoPackage: "github.com/malcolmston/express/sanitizehtml",
