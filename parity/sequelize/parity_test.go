// Package parity drives the upstream (sequelize over SQLite) and Go
// (github.com/malcolmston/sequelize) runners over an identical set of cases and
// compares their answers. Upstream is the oracle: nothing here hand-writes an
// expectation.
//
// Run with:
//
//	GOWORK=off go test ./parity/sequelize/
package parity
...
func TestParity(t *testing.T) {
	...
	if err != nil {
		t.Skip("node not found in PATH; skipping cross-language parity for sequelize")
	}
	if _, err := os.Stat(filepath.Join("node", "node_modules", "sequelize")); err != nil {
		t.Skip("parity/sequelize/node/node_modules/sequelize missing; run `npm install` in parity/sequelize/node")
	}
	...
	bin := filepath.Join(t.TempDir(), "sequelize-parity-go-runner")
	...
	t.Logf("sequelize parity: %d/%d cases match (%.2f%%), %d deviations, upstream %s, port %s",
...
func goModuleVersion(t *testing.T) string {
	...
	for _, line := range strings.Split(string(b), "\n") {
		...
		for i, w := range f {
			if w == "github.com/malcolmston/sequelize" && i+1 < len(f) {
				return w + "@" + f[i+1]
			}
		}
	}
