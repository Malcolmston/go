// Package parity drives the upstream (node-jsonwebtoken) and Go
// (github.com/malcolmston/jwt) runners over an identical set of cases and
// compares their answers. Upstream is the oracle: nothing here hand-writes an
// expectation.
//
// Run with:
//
//	GOWORK=off go test ./parity/jwt/
package parity
...
func TestParity(t *testing.T) {
	...
	if err != nil {
		t.Skip("node not found in PATH; skipping cross-language parity for jwt")
	}
	if _, err := os.Stat(filepath.Join("node", "node_modules", "jsonwebtoken")); err != nil {
		t.Skip("parity/jwt/node/node_modules/jsonwebtoken missing; run `npm install` in parity/jwt/node")
	}
	...
	bin := filepath.Join(t.TempDir(), "jwt-parity-go-runner")
	...
	up := startRunner(t, "node", exec.Command(node, filepath.Join("node", "run.js"), keys))
	gorun := startRunner(t, "go", exec.Command(bin, keys))
	...
	t.Logf("jwt parity: %d/%d cases match (%.2f%%), %d deviations, upstream %s, port %s",
...
func goModuleVersion(t *testing.T) string {
	...
	for _, line := range strings.Split(string(b), "\n") {
		...
		for i, w := range f {
			if w == "github.com/malcolmston/jwt" && i+1 < len(f) {
				return w + "@" + f[i+1]
			}
		}
	}
